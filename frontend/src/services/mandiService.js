...
async function getMandiMatchKeys(districtId, mandiId, mandis) {
  ...
  const selectedMandi = districtMandis.find((mandi) =>
    mandiKeysFor(mandi).includes(selectedKey)
  );
  ...
  if (districtId && supabase) {
    ...
    if (!error) {
      for (const alias of aliases ?? []) {
        keys.add(normalizeMandiKey(alias.source_mandi_id));
        keys.add(normalizeMandiKey(alias.source_mandi_name));
      }
    }
  }
...
export async function fetchMandiAvailability(districtId, cropId, mandiId) {
  ...
  const selectedMandi = mandis.find((m) => m.id === mandiId || m.mandiId === mandiId);
